    ...
    public static function convertNumber($number, $isRecursiveFromThousand = false) {
      ...
      if (abs($number) < $maxSize) {
        switch ($number) {
          ...
          case 1:
          case 2:
            $string = $isRecursiveFromThousand ? self::$words[$number."th"] : self::$words[$number];
            return $string;
        
          case $number < 21:
            $string = self::$words[$number];
            return $string;
          ...
          case $number < 100:
            ...
            $string = $prefix.self::$words["conc"].$suffix;
            return $string;
          ...
          case $number < pow(10,3):
            if ($number % 100 === 0) {
              ...
              $prefix = self::$words[$number - $number%100];
              $suffix = self::$words["conc2"] . self::convertNumber($number%100, $isRecursiveFromThousand);
              $string = $prefix.$suffix;
            }
            return $string;
        
          case $number < pow(10,6):
            $prefix = convertNumber(floor($number/pow(10,3)), true)." ";
            $lastDigit = substr(floor($number/pow(10,3)), -1);
            $lastTwoDigits = floor($number/pow(10,3)) > 9 ? substr(floor($number/pow(10,3)), -2) : "";
            
            if ($lastDigit == 1 && $lastTwoDigits != 11) {
              $prefix .= self::$words[$lang][1001];
            } elseif(($lastDigit == 2 || $lastDigit == 3 || $lastDigit == 4) &&
              $lastTwoDigits != 12 && $lastTwoDigits != 13 && $lastTwoDigits != 14) {
              $prefix .= self::$words[$lang][1002];
            } else {
              $prefix .= self::$words[$lang][1003];
            }
            $suffix = $number%pow(10,3) ? self::convertNumber($number%pow(10,3), $lang) : "";
            $string = $prefix . " " . $suffix;
            return $string;
          ...
          case $number < pow(10,9):
            ...
            if ($lastDigit == 1 && $lastTwoDigits != 11) {
              $prefix .= self::$words["m1"];
            } elseif(($lastDigit == 2 || $lastDigit == 3 || $lastDigit == 4) &&
              $lastTwoDigits != 12 && $lastTwoDigits != 13 && $lastTwoDigits != 14) {
              $prefix .= self::$words["m2"];
            } else {
              $prefix .= self::$words["m3"];
            }
              
            $suffix = $number%pow(10,6) ? self::convertNumber($number%pow(10,6), $lang) : "";
            $string = $prefix." ".$suffix;
            return $string;
        }
      } else {
        echo self::$words["error"].number_format($maxSize-1)." ".self::$words["to"]." ".number_format($maxSize-1);
      }
    }
  }
